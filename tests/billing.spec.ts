...
import { computeCosts, equivalentFreeTokens, formatMoney, round6 } from '../src/billing.js'
...
describe('computeCosts', () => {
  it('计算缓存命中与未命中的混合计费', () => {
    ...
    expect(costs.cost_baseline).toBeCloseTo(0.00634, 6)
    expect(costs.cost_actual).toBeCloseTo(0.00317, 6)
    expect(costs.savings).toBeCloseTo(0.00317, 6)
  })
  ...
  it('round6 入库保留 6 位小数（展示时再 toFixed(2)）', () => {
